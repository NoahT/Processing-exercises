...
void setup() {
  ...
  this.system = new ParticleSystem(new PVector(width / 2 - (particle.size.x / 2), height * .5));
...
void draw() {
  ...
  particle.size = new PVector(50, 10);
  particle.velocity = new PVector(map(random(1), 0, 1, -10, 10), map(random(1), 0, 1, 0, -20));
  particle.position = system.origin;//new PVector(width / 2 - (particle.size.x / 2), height * .5);
